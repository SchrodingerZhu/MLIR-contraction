// Pooling operation in MLIR affine dialect
...
func.func @pooling(%arg0: memref<?x?x?x?xf32>,     // Input tensor I[N][C][IH][IW]
                   %arg1: memref<?x?x?x?xf32>,     // Output tensor O[N][C][OH][OW]
                   %N: index, %C: index,
        ...
        affine.for %ow = 0 to %OW {
          affine.for %kh = 0 to %KH {
            affine.for %kw = 0 to %KW {
              ...
              affine.store %add, %arg1[%n, %c, %oh, %ow] : memref<?x?x?x?xf32>
            }
          }
        }
      }
